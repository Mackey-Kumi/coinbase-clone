...
export default function SignIn() {
  ...
  return (
    <AuthLayout>
      <h1 className="text-[24px] font-semibold text-white mb-6">Sign in to Coinbase</h1>

      <form onSubmit={handleSubmit} className="space-y-3">
        {/* Email label */}
        <label className="block">
          <span className="text-[14px] font-medium text-white mb-1.5 block">Email</span>
          <input
            type="email"
      ...
      <div className="space-y-3">
        <SSOButton icon={<PasskeyIcon />} label="Sign in with Passkey" />
        <SSOButton icon={<GoogleIcon />} label="Sign in with Google" />
        <SSOButton icon={<AppleIcon />} label="Sign in with Apple" />
      </div>

      {/* Sign up link */}
      <p className="text-center mt-6 text-[14px] text-white">
        Don&apos;t have an account?{' '}
        <Link to="/signup" className="text-[#4670CA] font-semibold hover:underline">
          Sign up
        </Link>
      </p>
    </AuthLayout>
